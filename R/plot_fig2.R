...
#' 
#' @import ggplot2
plot_fig2 <- function(results_dir, abslocs = get_chromosome_coordinates()) {
  dir.create(file.path(results_dir,'arms/plots'),showWarnings=T)
  dir.create(file.path(results_dir,'genes/plots'),showWarnings=T)
  armfiles <- list.files(file.path(results_dir,'arms','files'),pattern='*fig2.txt', full.names=T, recursive= T)
  
  genefiles <- list.files(file.path(results_dir,'genes','files'),pattern='*fig2.txt', full.names=T)
  for(ttpath in c(armfiles,genefiles)) {
    ...
    if(ttpath %in% armfiles) {
      ...
      curr_dir = file.path(results_dir,'arms/plots/', arm)
      if(! dir.exists(curr_dir)) {
        dir.create(curr_dir, showWarnings=T)
      }
    ...
    resize_widths <- function(g, widths = rep(1, length(idpanels))){
      ...
      g$widths[idpanels] <- grid::unit.c(do.call(unit, list(widths, 'null')))
      g
    }
    
    g1 <- ggplotGrob(p1)
    g2 <- ggplotGrob(p2)
    g0 <- cowplot::plot_grid(p1,p2,rel_widths=c(5,1.5),align='h')
    ...
    #if(strsplit(ttpath,'_')[[1]][4]=='amp' | strsplit(ttpath,'_')[[1]][4]=='del') {
    if(ttpath %in% armfiles) {
      ggsave(plot=g0, file.path(results_dir,'arms','plots',arm,str_replace(basename(ttpath),'txt','pdf')),height=4,width=11)
      
    } else {
      ggsave(plot=g0, file.path(results_dir,'genes','plots',str_replace(basename(ttpath),'txt','pdf')),height=4,width=11)
    }
  }
}
